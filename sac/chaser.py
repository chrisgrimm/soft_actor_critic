...
class ChaserEnv(Env):
    def __init__(self, visual=False, no_prey=False, max_steps=1000):
        ...
        self.surface = pygame.Surface((self.size * self.block_pixels,
                                       self.size * self.block_pixels))
    ...
    def get_position(self, positions_to_avoid=None):
        while True:
            ...
            if (positions_to_avoid is
                    None) or (position not in positions_to_avoid):
                return position
    ...
    def get_obs(self, agent_pos, prey_pos, visual):
        if visual:
            ...
            agent_rect = pygame.Rect(agent_pos,
                                     (self.block_pixels, self.block_pixels))
            if self.no_prey:
                pygame.draw.rect(self.surface, self.agent_color, agent_rect)
            else:
                prey_rect = pygame.Rect(prey_pos,
                                        (self.block_pixels, self.block_pixels))
                if self.agent_pos == self.prey_pos:
                    pygame.draw.rect(self.surface, self.overlap_color,
                                     agent_rect)
                else:
                    pygame.draw.rect(self.surface, self.agent_color,
                                     agent_rect)
            ...
            if self.no_prey:
                ...
                return np.concatenate(
                    [
                        ...
                        np.array(self.prey_pos) / self.size
                    ],
                    axis=0)
    ...
    def step(self, action):
        ...
        reward = -0.01
        self.step_num += 1
        self.agent_pos = self.update_position(self.agent_pos,
                                              self.action_dict[action])

        norm_agent, norm_prey = np.array(self.agent_pos) / self.size, np.array(
            self.prey_pos) / self.size
        ...
        if self.agent_pos == self.prey_pos and not self.no_prey:
            terminal = True
            #reward = 1
            return self.get_obs(self.agent_pos, self.prey_pos,
                                self.visual), reward, terminal, {}
    ...
    def get_random_batch(self, batch_size):
        ...
        for i in range(batch_size):
            sample = self.get_obs(self.get_position(), self.get_position(),
                                  self.visual)
            batch.append(sample)
