...
class MLPValueFunc(object):
    def Q_network(self, s, a, name, reuse=None):
        with tf.variable_scope(name, reuse=reuse):
            ...
            fc1 = tf.layers.dense(sa, 256, tf.nn.relu, name='fc1', kernel_initializer=tf.constant_initializer(np.random.uniform()))
            ...
            q = tf.reshape(tf.layers.dense(fc3, 1, name='q', kernel_initializer=tf.constant_initializer(np.random.uniform())), [-1])
        return q
    ...
    def V_network(self, s, name, reuse=None):
        with tf.variable_scope(name, reuse=reuse):
            ...
            fc2 = tf.layers.dense(fc1, 256, tf.nn.relu, name='fc2', kernel_initializer=tf.constant_initializer(np.random.uniform()))
            fc3 = tf.layers.dense(fc2, 256, tf.nn.relu, name='fc3', kernel_initializer=tf.constant_initializer(np.random.uniform()))
            v = tf.reshape(tf.layers.dense(fc3, 1, name='v', kernel_initializer=tf.constant_initializer(np.random.uniform())), [-1])
        return v
