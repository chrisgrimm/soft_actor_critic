...
class MLPPolicy(object):
    def input_processing(self, s):
        fc1 = tf.layers.dense(s, 256, tf.nn.relu, name='fc1', kernel_initializer=tf.random_uniform())
        fc2 = tf.layers.dense(fc1, 256, tf.nn.relu, name='fc2', kernel_initializer=tf.random_uniform())
        fc3 = tf.layers.dense(fc2, 256, tf.nn.relu, name='fc3', kernel_initializer=tf.random_uniform())
        return fc3
...
class GaussianPolicy(object):
    ...
    def produce_policy_parameters(self, a_shape, processed_s):
        mu_params = tf.layers.dense(processed_s, a_shape, name='mu_params', kernel_initializer=tf.random_uniform())
        sigma_params = tf.layers.dense(
            processed_s, a_shape, tf.nn.sigmoid, name='sigma_params', kernel_initializer=tf.random_uniform())
...
class CategoricalPolicy(object):
    def produce_policy_parameters(self, a_shape, processed_s):
        logits = tf.layers.dense(processed_s, a_shape, name='logits', kernel_initializer=tf.random_uniform())
        return logits
